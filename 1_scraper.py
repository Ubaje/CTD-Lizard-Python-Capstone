...
with c3:
    ...
    def best_stat(ev, stat_name, col_name):
        rows = ev[ev["statistic"] == stat_name][["player","team","league","value"]]
        if rows.empty:
            return pd.DataFrame(columns=["player_name","team","league", col_name])
        return (rows.groupby(["player","team","league"])["value"]
                    .max().reset_index()
    ...
    if df_x.empty or df_y.empty:
        st.info(f"No data found for {'  ' + x_stat_name if df_x.empty else y_stat_name}. "
                f"Try a different stat combination.")
    else:
        sc_data = df_x.merge(df_y, on=["player_name","team","league"], how="inner")
        ...
        if sc_data.empty:
            ...
            fig3 = px.scatter(
                ...
                color=color_col,
                hover_data=[c for c in ["player_name","team"] if c in sc_data.columns],
